...
# Set the native working directory for all following commands.
...
# Set the working directory inside the runtime container to match the PHP base image defaults.
WORKDIR /var/www/html

# Install nginx and curl for request handling.
RUN apt-get update && apt-get install -y \
    nginx \
    curl \
    && rm -rf /var/lib/apt/lists/*
